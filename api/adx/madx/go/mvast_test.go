...
func TestVAST_ToString(t *testing.T) {
	Convey("vast to string", t, func() {
		...
		xmlFile := `
					...
					<VAST version="2.0">
					  ...
					  <InLine>
						...
						<Error>http://myErrorURL/error2</Error>
						<Impression>http://myTrackingURL/impression</Impression>
						<Impression id="foo">http://myTrackingURL/impression2</Impression>
						<Creatives>
		...
		v, err := v.ToVAST(xmlFile)
		So(err, ShouldBeNil)
		s := v.Ads[0].InLine.AdTitle.CDATA
		t.Log(s)
		vastString, err := v.ToString()
		t.Log(vastString)
		So(vastString, ShouldContainSubstring, s)
	})
}
