...
type MContent struct {
	...
	Producer           *MProducer   `json:"producer,omitempty"`           // 内容提供者的详细信息
	...
	Data               []*MData     `json:"data,omitempty"`               // 附加内容数据
	Ext                *MContentExt `json:"ext,omitempty"`                // 内容扩展
}

type MContentExt struct {
	ChannelId int `json:"channelid,omitempty"` //频道，针对有些adx有频道的概念，增加此字段。
}
