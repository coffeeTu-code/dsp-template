...
type MBanner struct {
	...
	Format   []*MFormat  `json:"format,omitempty"`   // Array of format objects representing the banner sizes permitted.
	...
	Ext      *MBannerExt `json:"ext,omitempty"`      // 特定交易的OpenRTB协议的扩展信息占位符
}
type MBannerExt struct {
	PlacementType           string `json:"placementtype,omitempty"`           // 如果是激励广告，本属性值为字符串rewarded，默认为空.
	BannerPlayableOnly      bool   `json:"bannerplayableonly,omitempty"`      // If true, set the value of BidResponse.seatbid.bid.ext.crtype in your bid response to "MRAID playable".
	AllowsCustomCloseButton bool   `json:"allowscustomclosebutton,omitempty"` // If true allow custom close button or not.
}
