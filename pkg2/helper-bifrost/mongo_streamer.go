...
func NewMongoStreamer(mongoConfig *MongoStreamerCfg) (*MongoStreamer, error) {
	...
	ctx, _ := context.WithTimeout(context.TODO(), time.Duration(mongoConfig.MongoConfig.ConnectTimeout)*time.Microsecond)
	opt := options.Client().ApplyURI(mongoConfig.MongoConfig.URI)
	...
	d := time.Duration(mongoConfig.MongoConfig.ReadTimeout) * time.Microsecond
	...
	streamer.collection = client.Database(mongoConfig.MongoConfig.DB).Collection(mongoConfig.MongoConfig.Collection)
	if streamer.collection == nil {
		if mongoConfig.Logger != nil {
			mongoConfig.Logger.Warnf("[%s.%s] Not found", mongoConfig.MongoConfig.DB, mongoConfig.MongoConfig.Collection)
		}
		return nil, errors.New(fmt.Sprintf("[%s.%s] Not found", mongoConfig.MongoConfig.DB, mongoConfig.MongoConfig.Collection))
	}
...
func (ms *MongoStreamer) GetScheduleInfo() *ScheduleInfo {
	return &ScheduleInfo{
...
func (ms *MongoStreamer) loadBase(ctx context.Context) (err error) {
	for i := -1; i < ms.cfg.MongoConfig.TryTimes; i++ {
...
func (ms *MongoStreamer) loadInc(ctx context.Context) error {
	...
	c, _ := context.WithTimeout(ctx, time.Duration(ms.cfg.MongoConfig.ReadTimeout)*time.Microsecond)
